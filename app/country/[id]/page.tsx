...
export default function CountryPage({
  ...
  const [countries, setCountries] = useState<{ id: string; name: string; iso3: string }[]>([])
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    async function fetchData() {
      try {
        const [countryRes, listRes] = await Promise.all([
    ...
    <div className="flex flex-col min-h-screen p-4 lg:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
         ...
         <div className="w-full max-w-xs">
            <Select
              value={country.iso3}
              onValueChange={(value) => router.push(`/country/${value}`)}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                {countries.map((c) => (
                  <SelectItem key={c.id} value={c.iso3}>
                    {c.name}
                  </SelectItem>
                ))}
              </SelectContent>
